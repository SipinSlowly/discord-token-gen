...
function write_log(goodnews, text){
  ...
  const color = goodnews ? "\x1b[31m" : "\x1b[31m";

  console.log(`${color} [LOG] \x1b[37m ${text}`);
  logger.log(`[LOG - ${time}s] ${text}`);
}

// Code start there
write_log(true, "Generator Made By aj/a;#0003");
...
async function create_accinfos(browser, d) {
  ...
  let abeceda = "abcdefghijklmnopqrstuvwxyz123456789"
    let vysledek = ""
    for(let i=0;i<4;i++) {
    vysledek += abeceda[Math.floor(Math.random() * abeceda.length)]
}
  const username = vysledek + fs.readFileSync("Username.txt");
